      ...
      {Number.isFinite(lockedCakeBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  'Your vAlien boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.',
                )
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term Alien staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('Alien locked:')} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of Alien you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. Alien held in other places does not contribute to your voting power.',
        )}
      </Text>
