...
const StakeAction: React.FC<React.PropsWithChildren<FarmCardActionsProps>> = ({
  ...
  const crossChainWarningText = useMemo(() => {
    return isFirstTime
      ? t('A small amount of %nativeToken% is required for the first-time setup of cross-chain Alien farming.', {
  ...
  const [onPresentDeposit] = useModal(
    <FarmUI.DepositModal
      ...
      showCrossChainFarmWarning={chainId !== ChainId.ARBITRUM && chainId !== ChainId.BSC_TESTNET}
  ...
  if (!isApproved) {
    return (
      <Button mt="8px" width="100%" variant="AlienActive" disabled={pendingTx || isBloctoETH} onClick={handleApprove}>
        {t('Enable Contract')}
      </Button>
    )
  }
