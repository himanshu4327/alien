...
const Step1 = ({ hasProfile }: { hasProfile: boolean }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      <Text>
        {t(
          'The number of iAlien equals the locked staking amount if the staking duration is longer than %weeks% weeks. If the staking duration is less than %weeks% weeks, it will linearly decrease based on the staking duration.',
          {
            weeks: weeksDisplay,
          },
        )}
  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Lock Alien in the Alien pool')}
      </Heading>
      <Box>
        <Text mb="4px" color="textSubtle" small>
          {t(
            'The maximum amount of Alien you can commit to the Public Sale equals the number of your iAlien. Lock more Alien for longer durations to increase the maximum Alien you can commit to the sale.',
          )}
        </Text>
        <TooltipText as="span" fontWeight={700} ref={targetRef} color="textSubtle" small>
          {t('How does the number of iAlien calculated?')}
        </TooltipText>
        <Text mt="4px" color="textSubtle" small>
          {t(
            'Missed this IFO? You will enjoy the same amount of iAlien for future IFOs if your locked-staking position is not unlocked.',
          )}
        </Text>
      </Box>
      {hasProfile && (
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t(
          'Please note that Alien in the fixed-term staking positions will remain locked and can not be used for committing to IFO sales. You will need a separate amount of Alien in your wallet balance to commit to the IFO sales.',
        )}{' '}
        <br />
      </Text>
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent Alien tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
